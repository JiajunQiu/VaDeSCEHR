...
class DataGen(tf.keras.utils.Sequence):
    def __init__(self, X,batch_size=32,shuffle=True,pretrain=False):
...
def VAEData(dataframe, token2idx1,  token2idx2, token2idx3, age2idx, code_map, max_len):
    ...
    self_time = dataframe['eventtime']
    self_event = dataframe['event']
    self_multi = dataframe['multi']
    self_cluster = dataframe['cluster']
    self_code_map = code_map
    self_age2idx = age2idx


    Dataset={'code1':[], 'code2':[], 'code3':[], 'age':[], 'multi':[],'typ':[],'position':[], 'mask':[], 'label':[], 'event':[],'time':[],'cluster':[]}
    ...
    for index in range(len(self_code1)):
        ...
        code1,code2,code3, label = code_convert(code1,code2,code3, self_vocab1,self_vocab2,self_vocab3,self_code_map)
        ...
        tokens = seq_padding(code3, self_max_len)
