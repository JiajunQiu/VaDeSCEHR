...
class ModelEmbeddings(tf.keras.layers.Layer):
    def __init__(self, config):
        ...
        self.word_embeddings.build(input_shape=(None, config['vocab_size']))


        self.layer_norm = tf.keras.layers.LayerNormalization(epsilon=1e-12, axis=-1)
        self.dropout = tf.keras.layers.Dropout(0.1)

    def call(self, word_ids, posi_ids=None):
...
class ModelForMaskedLM(tf.keras.Model):
    def __init__(self, config, **kwargs):
        ...
        self.cls = ModelOnlyMLMHead(config, self.ModelATT.embeddings.word_embeddings.weights[0])
