...
def train(configs,trn_loader,val_loader,save=None):
    ...
    model.compile(opt, loss={"output_1": rec_loss}, metrics={"output_5": cindex_metric})

    history = model.fit(trn_loader, validation_data=val_loader, epochs=configs['training']['epoch'], verbose=2)

    val_loss = history.history['val_loss'][-1]
    val_rec_loss = history.history['val_output_1_loss'][-1]

    model.sample_surv = False

    rec, z_sample, p_z_c, p_c_z, risk_scores, lambdas, event_and_time = model.predict(val_loader)
    risk_scores = np.squeeze(risk_scores)
    c_hat = np.argmax(p_c_z, axis=-1)
    val_ci = cindex(t=event_and_time[:,0], d=event_and_time[:,1], scores_pred=risk_scores)

    prior_logits = tf.math.abs(model.prior_logits)
    norm = tf.math.reduce_sum(prior_logits, keepdims=True)
    prior = prior_logits / (norm + 1e-60)
    log_prior = tf.math.log(prior + 1e-60)
    log_likelihood = tf.reduce_logsumexp(log_prior + p_z_c, axis=-1)
    mean_nll = tf.reduce_mean(-1*log_likelihood)
    #BIC = -2 * LL + log(N) * k
    k=configs['training']['num_clusters']*(2*configs['training']['latent_dim'])+(configs['training']['num_clusters']-1)
    mean_bic = tf.reduce_mean(-2*log_likelihood+tf.math.log(tf.cast(len(risk_scores),tf.float32))*k)

    print(float(val_loss),float(val_rec_loss), float(val_ci), float(mean_nll),float(mean_bic),file=out,sep='\t')
    out.close()
...
gen_trn = DataGen(gen_trn, batch_size=batch_size,shuffle=True)
