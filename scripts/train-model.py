...
configs_path = os.path.join(work_path,'config.pkl')
configs=pickle.load(open(configs_path,'rb'))
...
def train(configs,trn_loader):
    ...
    history = model.fit(trn_loader, epochs=configs['training']['epoch'], verbose=2)
    ...
    model.save_weights(os.path.join(save_path, 'model_weights_overall'))
setup_seed(seed)
...
batch_size=100
fold='12345'
trn_data=data_concat(data_ori[int(fold[0])-1],data_ori[int(fold[1])-1],data_ori[int(fold[2])-1],data_ori[int(fold[3])-1],data_ori[int(fold[4])-1])
